...
<script>
        document.addEventListener("DOMContentLoaded", function () {
           ...
           function upDate(){
               ...
               data={
                        id:this.name,
                        blockade:document.getElementById(`blockade\${this.name}`).value
                    }
                    fetch("updateBlockade", { method: "POST", body: JSON.stringify(data), headers: { "Content-Type": "application/json" } }).
            ...
            function selectFor(data) {
                fetch("select", { method: "POST", body: JSON.stringify(data), headers: { "Content-Type": "application/json" } }).
                    ...
                    (function (data) {
                        if(data!=null){
                        con = `<table class='table table-striped'><thead><tr> <th scope='col'>姓名</th><th scope='col'>生日</th><th scope='col'>信箱</th><th scope='col'>性別</th><th scope='col'>職業</th><th scope='col'>停權</th><th scope='col'> </th></tr></thead><tbody>`
                        	counter=0;
                        	for(i=0;i<data.content.length;i++){
                            con+=`<tr><td>\${data.content[i].name}</td><td>\${data.content[i].birthday}</td><td>\${data.content[i].email}</td><td>\${data.content[i].gender}</td><td>\${data.content[i].job!=null?data.content[i].job.name:"無資料"}</td><td><select class="form-select" id='blockade\${data.content[i].id}'>`
                            con+=`<option value="0"`
							if(data.content[i].blockade==0){con+=` selected`}
                            con+=`>正常</option>`
                            con+=`<option value="1"`
							if(data.content[i].blockade==1){con+=` selected`}
                            con+=`>停權</option></select></td><td><button type="button" class="btn btn-primary" name="\${data.content[i].id}" id="upButton\${i}">送出</button></td></tr>`
                            counter++;
                            }
