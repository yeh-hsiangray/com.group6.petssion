...
<body
				...
				<div class="row mb-2">
					...
					<div class="col-sm-6">
						<ol class="breadcrumb float-sm-right" style="font-size: 26px;">
							...
							<li class="breadcrumb-item active">文章資源回收桶</li>
						</ol>
					</div>
				</div>
			</div>
							...
							<div class="col-6">
								<div class="form-group" style="font-size: 26px;">
									<label>檢索條件</label> <select class="select2"
										data-placeholder="檢索條件" id="searchOption"
						...
						<div class="form-group" style="font-size: 26px;">
							...
							<div class="input-group">
								...
								<div class="input-group-append">
									<button class="btn  btn-default" id="searchInBinButton"
					...
					<div class="col-12">
						<div class="card">
							<div class="card-header">
								<!-- /.card-header -->
								<div class="card-body">
									<a class="btn btn-info btn-sm" href="#" id="undoDelete"
										style="background-color: #3CB371; font-size: 26px;""> <i
										class="fas fa-trash-undo-alt"> </i> 復原選取
									</a>&nbsp&nbsp
									<a class="btn btn-danger btn-sm"
										style="background-color: darkred; font-size: 26px;"
										id="deleteArticleForever"> <i class="fas fa-trash-alt">
									</i>徹底刪除
									</a>&nbsp&nbsp
									<a class="btn btn-info btn-sm" href="#" id="viewAllInBin"
									...
									<table id="tbd" class="table table-bordered table-striped"
										...
										<thead>
											<tr>
												...
												<th width="32%;">標題</th>
												<th width="16%;">標籤</th>
												<th width="12.5%;">發文日期</th>
												<th width="12.5%;">修改日期</th>
											</tr>
										</thead>
										<tbody>
											<!-- <tr>
	...
	<script>
		...
		function getPageData(page,isSearch) {
			...
			$("#tbd tbody").empty();
		$("ul.pagination").empty();
		var searchUrl="<c:url value='/backend/blog/api/trashArticle'/>"+ "?page=" + page + "&size=" + pageSize;
		if(isSearch){
			searchUrl="<c:url value='/backend/blog/api/searchArticleInBin'/>"+ "?page=" + page + "&size=" + pageSize+
 ...
 $(document).ready(function(){
  ...
  $('#searchInBinButton').click(function(event){
	  event.preventDefault();
	  getPageData(1,true);
  });
  $('#showAllInBin').click(function(event){
	  event.preventDefault();
	  getPageData(1,false);
  });
  
  $('#undoDelete').click(function(event){
	  event.preventDefault();
	  var data={s1:0,s2:1}
	  updateStatus(data);
  });
  ...
  $('#deleteArticleForever').click(function(event){
	  event.preventDefault();
	  var data={s1:0,s2:-1}
	  updateStatus(data);
	})
